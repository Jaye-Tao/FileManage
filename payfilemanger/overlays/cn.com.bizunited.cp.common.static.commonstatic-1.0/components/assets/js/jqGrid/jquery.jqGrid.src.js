...
//Helper functions
	...
	createModal : function(aIDs, content, p, insertSelector, posSelector, appendsel, css) {
		var h = '<div id="errorTc" style="width:100%;height:100%;position:fixed;left:0;top:0;z-index:1000000;text-align:center; display:none;">' +
				    '<div style="width:100%;position:absolute;left:0;top:50%;">' +
				        '<div style="display:inline-block;background:rgba(0,0,0,.6);background:#000000 \\9;color:#ffffff; padding:10px 20px; border-radius:3px; margin-top:-10px;">' +
				         content + '</div>' +
				    '</div>' + 
				'</div>';
		$('body').append(h);
		$('#errorTc').fadeIn(200,function(){
			setTimeout(function(){
				$('#errorTc').fadeOut(200,function(){
					$('#errorTc').remove();
				});
			},1000);
		});
...
// Date Validation Javascript
	...
	checkValues : function(val, valref, customobject, nam) {
		...
		if(edtrul) {
			...
			if(edtrul.mobile === true && val !== '') {
				var _reg = /^13[0-9]{9}$|14[0-9]{9}|15[0-9]{9}$|18[0-9]{9}$|17[0-9]{9}$/;
				if( !_reg.test(val) )  { return [false,nm+": 格式有误",""]; }
			}
			// force required
			var rqfield = edtrul.required === false ? false : true;
			if(edtrul.number === true) {
				if( !(rqfield === false && $.jgrid.isEmpty(val)) ) {
					if(isNaN(val) || val === '') { return [false,nm+": "+$.jgrid.edit.msg.number,""]; }
				}
			}
	...
	saveCell : function (iRow, iCol){
		return this.each(function(){
			...
			if(fr !== null) {
				...
				// The common approach is if nothing changed do not do anything    v2 !== $t.p.savedRow[fr].v
				if (true){
